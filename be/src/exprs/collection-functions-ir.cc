...
// The THREAD_LOCAL state is always set up by ArrayContainsPrepare() before any row is
// evaluated. If preparation fails, OpenEvaluator() propagates the error and evaluation
// never runs, so the state is guaranteed to be non-null here.
ArrayContainsState* GetArrayState(FunctionContext* ctx) {
...
BooleanVal ArrayContainsPrimitive(FunctionContext* ctx, const CollectionVal& arr,
  ...
  ArrayContainsState* state = GetArrayState(ctx);
