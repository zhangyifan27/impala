...
/// 'result'. The range [start, end) must begin with quote_char. Returns, on success, the
/// number of characters consumed from start including both quotes; returns -1 if the
/// input does not start with quote_char, the closing quote is missing, or the arguments
/// are invalid. In all error cases 'result' is cleared. A backslash escapes the following
/// character (e.g. \" or \\) and the escaped character is appended to result literally.
